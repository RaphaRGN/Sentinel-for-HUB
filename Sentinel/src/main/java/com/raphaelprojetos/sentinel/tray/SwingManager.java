    ...
    public class SwingManager extends JFrame implements AlertaConsumer.ConsumerCallback {
        ...
        private void mostrarPopup(AlertaDTO alerta) {
            ...
            JLabel mensagemLabel = new JLabel("<html>" +
                    ...
                    "<p style='font-size:15px; text-align:center;'><strong>Código:</strong> " + alerta.getCodigo() + "</p>" +
                    "<p style='font-size:15px; text-align:center;'><strong>Descrição:</strong> " + alerta.getDescricao() + "</p>" +
                    "<p style='font-size:15px; text-align:center;'><strong>Data/Hora:</strong> " + alerta.getTempoFormatado() + "</p>" +
                    "</html>");
        ...
        private JPanel createConfigPanel() {
            ...
            // CheckBox para Modo Dashboard
            JCheckBox checkBoxDashborad = new JCheckBox("Modo Dashboard");
            checkBoxDashborad.setAlignmentX(java.awt.Component.CENTER_ALIGNMENT);
            checkBoxDashborad.setFont(new Font("Arial", Font.PLAIN, 14));
            contentPanel.add(checkBoxDashborad);
            contentPanel.add(Box.createRigidArea(new Dimension(0, 15)));

            // Lógica para ativar ou desativar o modo Dashboard
            checkBoxDashborad.addActionListener(e -> {
                if (checkBoxDashborad.isSelected()) {
                    JOptionPane.showMessageDialog(panelConfig,
                            "Modo Dashboard ativo! Agora o painel irá atualizar de 20 em 20 minutos, ideal para colocar na TV!");

                    tempoRestartDashboard = new Timer(20000, event -> atualizarTabelaAlertas());
                    tabelaAlertas.setSize(1920,800);
                    atualizarTabelaAlertas();
                    tempoRestartDashboard.start();
                } else {
                    if (tempoRestartDashboard != null && tempoRestartDashboard.isRunning()) {
                        tempoRestartDashboard.stop();
                        atualizarTabelaAlertas();
                        tempoRestartDashboard = null;
                        JOptionPane.showMessageDialog(panelConfig, "Modo Dashboard desativado!");
                    }
                }
            });
